...
$choice = $choose | Out-GridView -passthru 
...
elseif ($choice -eq $2)
    {$result = 2}
     ...
     While ($true) {   
        Write-Host "Nie wybrałeś żadnej z dostępnych opcji. Czy chcesz spróbować jeszcze raz? (y/n)"
        $sowhichisitdamnit = Read-Host
        if ($sowhichisitdamnit -eq "y")
            {break}
        elseif ($sowhichisitdamnit -eq "n")
            {$result = 3
            break} 
        else {continue}       
    }
...
foreach ($group in $Gselected){
    ...
    $scope = $group.GroupScope #pośrednik jest ewidentnie potrzebny
    $name = $group.Name
...
switch ($sowhichisit) {
1 {juzerzy
    Write-Host "Your report waits on your Desktop. Please press enter to exit"
    Read-Host}
2 {grupy
    Write-Host "Your report waits on your Desktop. Please press enter to exit"
    Read-Host}
3 { Write-Host "Oh well :/"
    Write-Host "Bye-bye"
    Write-Host ""
